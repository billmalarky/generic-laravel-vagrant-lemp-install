...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.hostname = "generic.local"

  # INTERNAL NETWORK - The IP to use when automatically configuring your hosts file
  config.vm.network :private_network, ip: "192.168.100.100"
  #config.vm.network "forwarded_port", guest: 80, host: 8080

  # SUBDOMAINS - Uncomment the line below to configure any necessary subdomains (e.g. blog.generic.local)
  # config.hostsupdater.aliases = ["www.generic.local"]
